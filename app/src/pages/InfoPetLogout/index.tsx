...
type ScreenRouteProp = RouteProp<RootStackParamList, "InfoPetLogout">;

interface Props {
  route: ScreenRouteProp;
}

export default function InfoPetLogout({ route }: Props) {
  ...
  const handleAdopt = () => {
    let url =
      "whatsapp://send?text=" +
      `Olá, gostaria de informações sobre o/a pet de nome ${pet.name}` +
      "&phone=91" +
      pet.user?.phone;
    Linking.openURL(url)
      .then((data) => {
    ...
    <View style={styles.background}>
      <View style={styles.imageContainer}>
        {pet.images && pet.images.length > 0 ? (
          <ImageViewer images={pet.images.map((img, index) => img.name)} />
        ) : (
